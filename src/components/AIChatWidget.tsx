...
import { fetchProductByHandle, markup } from "@/lib/shopify";
  ...
  const handleAdd = useCallback(async (e: React.MouseEvent) => {
    ...
    try {
      ...
      await addItem({
        ...
        price: { amount: (parseFloat(firstVariant.price.amount) + markup).toFixed(2), currencyCode: firstVariant.price.currencyCode },
        quantity: 1,
