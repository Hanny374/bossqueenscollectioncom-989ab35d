...
export const QuickViewModal = ({ product, open, onOpenChange }: QuickViewModalProps) => {
  ...
  const content = <QuickViewContent product={product} onOpenChange={onOpenChange} />;
...
function QuickViewContent({ product, onOpenChange }: { product: ShopifyProduct; onOpenChange: (open: boolean) => void }) {
  ...
  return (
    <>
      {/* Image Gallery */}
      <div className={`relative ${isMobile ? "aspect-[4/3] w-full" : "aspect-square"} bg-secondary/30`}>
        {images.length > 0 ? (
          <img
      ...
      {/* Product Info */}
      <div className={`${isMobile ? "p-4" : "p-6"} flex flex-col gap-3 ${isMobile ? "" : "max-h-[500px] overflow-y-auto"}`}>
        <div>
          <h2 className={`font-display ${isMobile ? "text-lg" : "text-xl"} font-bold text-foreground leading-tight mb-1`}>
            {node.title}
          </h2>
        ...
        <div className="flex flex-col gap-1 bg-secondary/50 rounded-xl p-2.5">
          ...
          <div className="flex items-center gap-2 text-xs text-muted-foreground">
            ...
            <span className="text-[11px]">US & Caribbean: 5–10 days · Worldwide: 10–21 days</span>
          </div>
        </div>
        ...
        {laceTypes.length > 0 && (
          <div className="space-y-1.5">
            <span className="text-xs font-medium text-foreground">Lace Type</span>
            <div className="flex flex-wrap gap-1.5">
              {laceTypes.map((lace) => (
                <button
                  key={lace}
        ...
        <div className={`flex gap-2 ${isMobile ? "sticky bottom-0 bg-background pt-3 pb-2 -mx-4 px-4 border-t border-border/30" : "mt-auto pt-4"}`}>
          <Button
        ...
        <Link
          ...
          className="flex items-center justify-center gap-1.5 text-sm text-primary font-medium hover:underline pb-2"
        >
          View Full Details
