...
const ProductPage = () => {
  ...
  const getAdjustedPrice = () => {
    const basePrice = parseFloat(selectedVariant?.price.amount || "0") + markup;
  ...
  useEffect(() => {
    ...
    addToRecentlyViewed({
      ...
      price: (parseFloat(product.priceRange.minVariantPrice.amount) + markup).toFixed(2),
  ...
  const productJsonLd = {
    ...
    offers: {
      ...
      lowPrice: (parseFloat(product.priceRange.minVariantPrice.amount) + markup).toFixed(2),
      highPrice: ((parseFloat(variants[variants.length - 1]?.node.price.amount || product.priceRange.minVariantPrice.amount)) + markup).toFixed(2),
  ...
  return (
    <div className="min-h-screen bg-background">
      <Helmet>
        ...
        <meta property="product:price:amount" content={(parseFloat(product.priceRange.minVariantPrice.amount) + markup).toFixed(2)} />
            ...
            <div className="space-y-6">
              <div>
                ...
                <div className="flex items-center gap-3 flex-wrap">
                  ...
                  {compareAtPrice && (parseFloat(compareAtPrice.amount) + markup) > adjustedPrice && (
                    <>
                      ...
                      <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full bg-destructive/10 text-destructive text-sm font-bold">
                        ...
                        Save {Math.round((((parseFloat(compareAtPrice.amount) + markup) - adjustedPrice) / (parseFloat(compareAtPrice.amount) + markup)) * 100)}%
                      </span>
                    </>
                  )}
                </div>
