-- Remove approved duplicate reviews that repeat the same normalized body text.
WITH ranked AS (
  SELECT
    id,
    ROW_NUMBER() OVER (
      PARTITION BY lower(regexp_replace(regexp_replace(trim(coalesce(body, '')), '[^[:alnum:]]+', ' ', 'g'), '\s+', ' ', 'g'))
      ORDER BY
        CASE WHEN photos IS NOT NULL AND cardinality(photos) > 0 THEN 0 ELSE 1 END,
...
-- Prevent future imports from creating the same approved review text again.
CREATE UNIQUE INDEX IF NOT EXISTS reviews_approved_body_dedupe_uidx
ON public.reviews (
  lower(regexp_replace(regexp_replace(trim(coalesce(body, '')), '[^[:alnum:]]+', ' ', 'g'), '\s+', ' ', 'g'))
)
WHERE status = 'approved' AND length(trim(coalesce(body, ''))) > 0;

-- Return only unique Loox review bodies for product review displays.
